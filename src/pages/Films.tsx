...
const Films: React.FC = () => {
  ...
  const handleFilmClick = useCallback(async (film: Film) => {
    ...
    try {
      ...
      if (mountedRef.current) setLoadingStates(prev => new Map(prev.set(film.image, false)));
    }
  }, []);
  ...
  const handleLinkClick = useCallback((film: Film) => {
    if (film.link) window.open(film.link, '_blank');
  }, []);
