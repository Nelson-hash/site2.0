    ...
    >
      <AnimatePresence mode="wait">
        {showText ? (
          ...
          <motion.div
            key="svg"
            /* AJOUT ICI : Les classes -translate-x-3 md:-translate-x-6 décalent le logo vers la gauche pour centrer visuellement le corps du kangourou */
            className={`${isMobile ? 'w-80 h-80 sm:w-[28rem] sm:h-[28rem] -translate-x-3' : 'w-96 h-96 md:w-[30rem] md:h-[30rem] lg:w-[40rem] lg:h-[40rem] -translate-x-5 md:-translate-x-6'} pointer-events-auto flex items-center justify-center`}
            initial={{ opacity: 0, scale: 0.8, filter: "blur(10px)" }}
            animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }}
            exit={{ opacity: 0, scale: 0.8, filter: "blur(10px)" }}
            whileHover={{ scale: isMobile ? 1 : 1.05 }}
