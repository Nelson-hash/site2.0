    ...
    >
      <AnimatePresence mode="wait">
        {showText ? (
          <motion.h1 
            key="text"
            className={`${isMobile ? 'text-5xl sm:text-6xl' : 'text-7xl md:text-8xl lg:text-9xl'} font-bold tracking-widest pointer-events-auto text-center`}
          ...
          <motion.div
            key="svg"
            className={`${isMobile ? 'w-64 h-64 sm:w-80 sm:h-80' : 'w-80 h-80 md:w-96 md:h-96 lg:w-128 lg:h-128 xl:w-144 xl:h-144'} pointer-events-auto flex items-center justify-center`}
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            whileHover={{ scale: isMobile ? 1 : 1.1 }}
